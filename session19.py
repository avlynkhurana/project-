...
def add_customer():
    ...
    return render_template('success.html',message = "{} added sucessfully".format(customer_data['name']))
...
def delete_customer(id):
    ...
    customer = db.fetch(query)[0]
    db.delete(query)
    return render_template('success.html', message="Customer with ID {} and name {} Deleted...".format(id, customer['name']))
...
def add_pet(id):
    ...
    return render_template('add-pet.html', email=session['vet_email'],
                           name=session['vet_name'],customer=customer)
...
def add_consultation(id):
    db = MongoDBHelper(collection="pet")
    query ={'_id': ObjectId(id)}
    pets = db.fetch(query)[0]
    pet = pets[0]
    return render_template('add-consultation.html', vet_id=session['vet_id'],email=session['vet_email'],
...
def save_consultation():
    ...
    if len(pet_data['problem']) == 0 or len(consultation_data['medicines']) == 0:
    ...
    return render_template('success.html', message="Consultation for Pet {} successfully.."
...
def search_customer():
    ...
    if len(customers) == 1:
        customer = customers[0]
        return render_template("customer-profile.html",customer=customer,email=session['vet_email'],name=session['vet_name'])
    else:
        return render_template("error.html", message="customer not found.....")


def main():
    web_app.secret_key = 'vets_app-key-1'
    web_app.run()
